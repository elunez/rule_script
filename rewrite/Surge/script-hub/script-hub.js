...
<html lang="zh-CN">
  ...
  <head>
    ...
    <link rel="icon" type="image/png" href="https://cdn.jsdelivr.net/gh/elunez/rule_script@main/rewrite/Surge/script-hub/assets/icon.png" />
    <link rel="apple-touch-icon" href="https://cdn.jsdelivr.net/gh/elunez/rule_script@main/rewrite/Surge/script-hub/assets/icon-dark.png">
    ...
    computed: {
      ...
      result: function () {
        ...
        if (src && target && type) {
          ...
          const cleanSrc = src.replace(/#.*$/, '').replace(/😂/g, '%F0%9F%98%82')
          const query = Object.keys(fields).map(i => i + '=' + encodeURIComponent(fields[i])).join('&')
          const shortPathFields = new Set(['type', 'target', 'del', 'jqEnabled'])
          const canUseShortPath = pathType === 'file' && fields.type === 'rule-set' && fields.target === 'surge-rule-set' && Object.keys(fields).every(i => shortPathFields.has(i))

          if (canUseShortPath) {
            const shortFlags = []
            if (fields.del === true || fields.del === 'true') shortFlags.push('del')
            if (fields.jqEnabled === true || fields.jqEnabled === 'true') shortFlags.push('jq')
            return this.baseUrl + pathType + '/_start_/rule-set/surge-rule-set/' + shortFlags.join('/') + (shortFlags.length ? '/' : '') + '_url_/' + cleanSrc + '/_end_/' + encodeURIComponent(filename) + suffix
          }

          return this.baseUrl + pathType + '/_start_/' + cleanSrc + '/_end_/' + encodeURIComponent(filename) + suffix + '?' + query
